...
def test_get_scenario_by_id(setup_database):
    """Test retrieving scenario by ID"""
    # First create a scenario
    with patch('app.services.lm_studio.LMStudioService.generate_scenario') as mock_generate:
        mock_generate.return_value = {
            ...
            "patient_background": "Test background",
            "medical_context": "Test context",
            "communication_challenge": "Test challenge",
            "learning_objectives": ["Test objective"],
            "category": "general",
            "difficulty_level": "beginner"
        }
        
        create_response = client.post("/api/v1/scenarios/generate", json={
            "category": "general",
            "difficulty_level": "beginner"
        })
...
def test_get_scenario_not_found(setup_database):
    """Test scenario not found error"""
...
def test_list_scenarios_empty(setup_database):
...
def test_list_scenarios_with_data(setup_database):
    """Test listing scenarios with existing data"""
    # Create a scenario first
    with patch('app.services.lm_studio.LMStudioService.generate_scenario') as mock_generate:
        mock_generate.return_value = {
            "title": "List Test Scenario",
        ...
        client.post("/api/v1/scenarios/generate", json={
            "category": "general",
            "difficulty_level": "beginner"
        })
        ...
        data = response.json()
        assert len(data) >= 1
        assert any(scenario["title"] == "List Test Scenario" for scenario in data)
...
def test_get_scenario_categories(setup_database):
    ...
    expected_categories = ["general", "pediatrics", "emergency", "surgery", "mental_health"]
...
def test_generate_scenario_validation_error(setup_database):
    """Test scenario generation with invalid data"""
...
def test_generate_scenario_with_context(setup_database):
    """Test scenario generation with additional context"""
    mock_lm_response = {
        "title": "Context-Based Scenario",
        "description": "Scenario with specific context",
        "patient_background": "Context-specific background",
        "medical_context": "Context medical info",
        "communication_challenge": "Context challenge",
    ...
    with patch('app.services.lm_studio.LMStudioService.generate_scenario') as mock_generate:
        ...
        response = client.post("/api/v1/scenarios/generate", json={
            "category": "pediatrics",
            "difficulty_level": "advanced",
            "context": "Dealing with anxious parents of a sick child"
        })
        
        assert response.status_code == 200
        data = response.json()
        assert data["title"] == "Context-Based Scenario"
        assert data["category"] == "pediatrics"
